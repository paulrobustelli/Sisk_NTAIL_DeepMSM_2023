...
class LitNet(pl.LightningModule):
    def __init__(self, epsilon, learning_rate, lagtime, batch_size):
        super().__init__()
        self.model = chi()
    ...
    def training_step(self, batch, batch_idx):
        b0, bt = batch[...,0],batch[...,1]; b0,bt = self.model(b0), self.model(bt)
...
def main(n_epochs, data_path, lagtime, epsilon, batch_size, stride,
         ...
         ckpt_path, training_method, indices = None, n_devices = None):
    ...
    logger = logger_(save_dir = "./", name = filename) #logger makes a directory, in this function name means directory name
    
    early_stop_ = EarlyStopping(monitor = "val_loss", patience = patience,check_finite = True, mode = "min")
    
    model_check_ = ModelCheckpoint(save_top_k = 1,dirpath="./"+filename,save_last = True, monitor = "val_loss", mode = "min", filename = f"version_{logger.version}")
    ...
    np.save(filename+"/indices", datamodule.data_gen.idx)
    return 
...
if __name__ == '__main__':
    ...
    filename = (f"""max_epochs:{args.n_epochs}.lagtime:{args.lagtime}.epsilon:{args.epsilon}.batch_size:{args.batch_size}."""
               f"""accum_grad_batches:{args.accumulate_grad_batches}.stride:{args.stride}.learning_rate:{args.learning_rate}""")
    if not os.path.exists(filename):
        os.makedirs(filename)
    #run the main function to train the model
    main(n_epochs = args.n_epochs, data_path = args.data_path, lagtime = args.lagtime, epsilon = args.epsilon, batch_size = args.batch_size,
        stride = args.stride, learning_rate = args.learning_rate, patience = args.patience, accumulate_grad_batches=args.accumulate_grad_batches,
        filename = filename, ckpt_path = args.ckpt_path, indices = args.indices, n_devices = args.n_devices, training_method = args.distributed_training_method)
